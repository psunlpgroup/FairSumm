...
class A2CU():
    ...
    def acu_generation_from_list(self, ref: List[str], output_path: str, batch_size: int):
        """
        generate ACUs from reference summaries and save to output_path as jsonl
        ...
        with torch.no_grad():
            with open(output_path, "a") as f:
                for batch in tqdm(dataloader, total=num_batches):
                    ...
                    dec = [self.generation_tok.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False)
                           for g in summaries]
                    for (i, x) in enumerate(dec):
                        print(json.dumps({
                            "acus": x.split("|||"),
    ...
    def score(self, references: List[str], candidates: List[str], generation_batch_size: int=1, matching_batch_size: int=1,
               output_path: str=None, precision_only: bool=True, verbose: bool=True, acu_path: str=None):
        """
        ...
        Args:
            ...
            matching_batch_size: the batch size for ACU matching
            output_path: the path to save the output scores
            precision_only: whether to only compute the precision score
            verbose: whether to print the scores
        ...
        if precision_only:
            scores = self.__recall_score(candidates, references, generation_batch_size, matching_batch_size, output_path, verbose, acu_path)
            if verbose:
                print(f"Recall score: {np.mean(scores).item():.4f}")
            return scores
        recall_scores = self.__recall_score(references, candidates, generation_batch_size, matching_batch_size, None, verbose,acu_path)
        prec_scores = self.__recall_score(candidates, references, generation_batch_size, matching_batch_size, None, verbose,acu_path)
